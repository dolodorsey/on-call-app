...
export default function OnCallMarketplace() {
  ...
  return <div className="oc2-app">
    <header className="oc2-topbar"><Brand/><button className="oc2-avatar" onClick={()=>setTab('profile')}>{profile?.full_name?.[0]||session.user.email?.[0]||'U'}</button></header>
    <main className="oc2-content">
      {tab==='home'&&<>
        <section className="oc2-home-hero"><div className="oc2-city-grid"><i/><i/><i/><i/><i/></div><div className="oc2-home-copy"><span>{new Date().getHours()<12?'GOOD MORNING':new Date().getHours()<18?'GOOD AFTERNOON':'GOOD EVENING'}</span><h1>What do you<br/>need done?</h1><p>Book now, schedule later, or set it to repeat.</p></div><button className="oc2-search-launch" onClick={()=>setTab('services')}><span>⌕</span><strong>Search 72 services</strong><em>›</em></button></section>
      ...
      {tab==='profile'&&<section className="oc2-screen"><div className="oc2-profile"><div>{profile?.full_name?.split(' ').map((part:string)=>part[0]).slice(0,2).join('')||'OC'}</div><span>CUSTOMER ACCOUNT</span><h1>{profile?.full_name||session.user.email}</h1><p>{session.user.email}</p></div><div className="oc2-menu"><button><span>⌂</span><div><strong>Saved addresses</strong><small>Home, work, and service locations</small></div><em>›</em></button><button><span>▣</span><div><strong>Payment methods</strong><small>Secure service authorizations</small></div><em>›</em></button><button><span>↻</span><div><strong>Recurring services</strong><small>Manage weekly and monthly work</small></div><em>›</em></button><a href="/apply"><span>＋</span><div><strong>Become a provider</strong><small>Apply to earn through ON CALL</small></div><em>›</em></a><button className="danger" onClick={async()=>{await supabase.auth.signOut();setSession(null);setProfile(null);setBookings([])}}><span>↪</span><div><strong>Sign out</strong><small>End this session</small></div><em>›</em></button></div></section>}
    </main>
