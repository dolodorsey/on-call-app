...
type Quote={canSettle:boolean;remainingMinutes:number;waitMinutes:number;fee:number;providerCompensation:number;reason?:string}
...
export default function ProviderNoShowHost(){
 ...
 const load=async()=>{
  ...
  if(!error&&!data?.error)setQuote(data as Quote);else setQuote(null)
 }
 ...
 const settle=async()=>{
  ...
  if(!window.confirm(`Close this job as a customer no-show?\n\nON CALL will capture a $${Number(quote.fee||0).toFixed(2)} no-show fee. Your compensation is $${Number(quote.providerCompensation||0).toFixed(2)}.`))return
 ...
 const remaining=Math.max(0,Number(quote?.remainingMinutes||0))
 return <div style={{position:'fixed',left:18,bottom:190,zIndex:1280,display:'grid',gap:7}}>
  {job&&quote&&!quote.canSettle&&<div style={{padding:'10px 13px',borderRadius:999,background:'rgba(11,23,39,.95)',color:'#fff',border:'1px solid rgba(255,255,255,.12)',fontSize:10,fontWeight:900,boxShadow:'0 12px 36px rgba(0,0,0,.28)'}}>CUSTOMER NO-SHOW · {remaining>0?`${remaining} MIN`:quote.reason||'NOT READY'}</div>}
