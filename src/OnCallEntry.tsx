...
export default function OnCallEntry(){
  ...
  const filtered=useMemo(()=>{
    ...
    return services.filter(service=>(category==='all'||service.category_id===category)&&(!needle||`${service.name} ${service.description||''}`.toLowerCase().includes(needle)))
  },[services,query,category])
  const popular=useMemo(()=>services.filter(service=>service.on_demand_available).slice(0,8),[services])
  ...
  return <div className="oc-entry">
    ...
    <header className="oce-top"><div className="oce-wrap">
      <nav className="oce-nav"><div className="oce-logo"><i>OC</i><span>ON CALL</span></div><div className="oce-nav-actions"><button className="oce-ghost" onClick={()=>location.assign('/apply')}>Become a provider</button><button className="oce-solid" onClick={()=>openAuth()}>Sign in</button></div></nav>
      <div className="oce-hero"><div className="oce-kicker">72 SERVICES · ONE BUTTON</div><h1>Everyday life.<br/>Handled.</h1><p>Cleaning, repairs, beauty, auto, events, wellness, errands, tech, moving and premium concierge — one marketplace built around what you need done.</p><div className="oce-search"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search services — barber, cleaning, handyman…"/><button onClick={()=>document.getElementById('services')?.scrollIntoView({behavior:'smooth'})}>Search</button></div></div>
    </div></header>

    <main className="oce-main">
      <div className="oce-statbar"><div className="oce-stat"><strong>{services.length||72}</strong><span>LIVE SERVICE TYPES</span></div><div className="oce-stat"><strong>{categories.length||12}</strong><span>LIFESTYLE CATEGORIES</span></div><div className="oce-stat"><strong>10</strong><span>LAUNCH MARKETS</span></div></div>

      {!query&&<section className="oce-section"><div className="oce-title"><div><h2>Popular right now</h2><small>Services that can be requested on demand</small></div></div><div className="oce-grid">{popular.map(service=><button key={service.id} className="oce-card" onClick={()=>openAuth(service.name)}><div className="oce-card-top"><h3>{service.name}</h3><span className="oce-price">{money(service.base_price)}</span></div><p>{service.description}</p><span className="oce-meta">{service.pricing_unit} · {service.duration_minutes||'—'} min</span></button>)}</div></section>}

      <section className="oce-section" id="services"><div className="oce-title"><div><h2>{query?'Search results':'Browse everything'}</h2><small>{filtered.length} services available in the catalog</small></div></div><div className="oce-cats"><button className={`oce-chip ${category==='all'?'active':''}`} onClick={()=>setCategory('all')}>All services</button>{categories.map(item=><button key={item.id} className={`oce-chip ${category===item.id?'active':''}`} onClick={()=>setCategory(item.id)}>{item.name}</button>)}</div><div style={{height:14}}/>{filtered.length?<div className="oce-grid">{filtered.map(service=><button key={service.id} className="oce-card" onClick={()=>openAuth(service.name)}><div className="oce-card-top"><h3>{service.name}</h3><span className="oce-price">{money(service.base_price)}</span></div><p>{service.description}</p><span className="oce-meta">{service.on_demand_available?'On demand + scheduled':'Scheduled'} · {service.pricing_unit}</span></button>)}</div>:<div className="oce-empty">No service matches that search. Try a broader term.</div>}</section>
