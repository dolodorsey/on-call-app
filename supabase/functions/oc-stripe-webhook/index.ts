...
async function constructEvent(raw: string, signature: string) {
  ...
  for (const secret of secrets) {
    try {
      ...
      // Try the next registered endpoint secret.
    }
  }
  ...
  if (payment) {
    if (event.type === "payment_intent.amount_capturable_updated") {
      ...
      await client.from("oc_booking_payments").update({
        ...
        capture_by: charge?.payment_method_details?.card?.capture_before
          ? new Date(charge.payment_method_details.card.capture_before * 1000).toISOString()
          : null,
        updated_at: new Date().toISOString(),
      }).eq("id", payment.id);
    } else if (event.type === "payment_intent.succeeded") {
