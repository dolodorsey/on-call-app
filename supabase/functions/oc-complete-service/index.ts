...
Deno.serve(async req=>{
  ...
  try{
    ...
    await admin.from("oc_booking_payments").update({status:"capture_pending",failure_message:null,updated_at:now}).eq("id",payment.id).in("status",["authorized","capture_pending"]);
    try{const intent=await stripe.paymentIntents.capture(payment.stripe_payment_intent_id,{},{idempotencyKey:`oc-payment-${payment.id}-capture-v2`});return json({ok:true,booking:{id:bookingId,status:"completed"},payment:{status:intent.status,captured:true}},200,origin);}catch(error){console.error("ON CALL service capture failed",{bookingId,paymentId:payment.id,error:String(error)});await admin.from("oc_booking_payments").update({status:"authorized",failure_message:"Automatic capture retry queued",updated_at:new Date().toISOString()}).eq("id",payment.id).eq("status","capture_pending");return json({ok:true,booking:{id:bookingId,status:"completed"},payment:{status:"authorized",captured:false,settlement:"pending_retry"},warning:"Service is complete. Payment capture will retry automatically; no provider action is required."},202,origin);}
