...
Deno.serve(async req=>{
  ...
  if(booking.pricing_status!=='confirmed')return json({error:'Provider final price confirmation is required before payment authorization'},409,origin);
  ...
  if(!Number.isSafeInteger(serviceAmount)||serviceAmount<50||!Number.isSafeInteger(amount)||amount<50)return json({error:'Booking total is invalid'},409,origin);
  ...
  const platformFee=Math.round(serviceAmount*feeBps/10000),providerAmount=serviceAmount-platformFee;
  ...
  if(existing?.stripe_checkout_session_id){try{const prior=await stripe.checkout.sessions.retrieve(existing.stripe_checkout_session_id);if(prior.status==='open'&&prior.url&&Number(existing.amount_authorized)===amount)return json({checkoutUrl:prior.url,checkoutSessionId:prior.id,paymentId:existing.id,status:existing.status},200,origin);if(prior.status==='open'&&Number(existing.amount_authorized)!==amount)await stripe.checkout.sessions.expire(prior.id);if(prior.status==='complete')return json({alreadyAuthorized:['authorized','capture_pending','captured','transfer_pending','transferred'].includes(existing.status),status:existing.status,paymentId:existing.id,checkoutSessionId:prior.id},200,origin)}catch(error){console.warn('Prior ON CALL checkout lookup failed',String(error))}}
  ...
  const session=await stripe.checkout.sessions.create({mode:'payment',success_url:success,cancel_url:cancel,line_items:[{price_data:{currency:'usd',unit_amount:amount,product_data:{name:`ON CALL — ${booking.service_name}`,metadata:{booking_id:booking.id,service_amount_cents:String(serviceAmount),tax_cents:String(taxCents)}}},quantity:1}],customer_email:booking.customer?.email||user.email||undefined,payment_intent_data:{capture_method:'manual',transfer_group:`oc_booking_${booking.id}`,metadata:{brand:'ON_CALL',flow:'booking',booking_id:booking.id,customer_id:booking.customer_id,provider_id:booking.provider_id,service_amount_cents:String(serviceAmount),tax_cents:String(taxCents)}},metadata:{brand:'ON_CALL',flow:'booking',booking_id:booking.id,customer_id:booking.customer_id,provider_id:booking.provider_id}},{idempotencyKey:`oc-checkout-${booking.id}-${amount}-v2`});
  ...
  if(paymentError){try{if(session.status==='open')await stripe.checkout.sessions.expire(session.id)}catch{};console.error('ON CALL hosted checkout payment ledger failed',paymentError);return json({error:'Payment authorization could not be prepared. No active Checkout was left open.'},500,origin)}
