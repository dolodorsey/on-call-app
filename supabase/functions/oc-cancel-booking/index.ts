  ...
  if(payment){
    ...
    if(payment.stripe_payment_intent_id){
      try{
        ...
        if(feeCents===0){
          const pi=await stripe.paymentIntents.cancel(payment.stripe_payment_intent_id,{cancellation_reason:'requested_by_customer'},{idempotencyKey:`oc-cancel-booking-${payment.id}-free-v1`});
          await admin.from('oc_booking_payments').update({status:'authorization_canceled',stripe_payment_intent_id:pi.id,canceled_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',payment.id);
        }else{
          const pi=await stripe.paymentIntents.capture(payment.stripe_payment_intent_id,{amount_to_capture:feeCents,final_capture:true,metadata:{settlement_type:'customer_cancellation',cancellation_fee_cents:String(feeCents)}},{idempotencyKey:`oc-cancel-booking-${payment.id}-${feeCents}-v2`});
