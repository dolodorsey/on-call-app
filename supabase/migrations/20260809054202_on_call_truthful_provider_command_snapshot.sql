...
declare
  v_provider public.oc_provider_profiles%rowtype; v_user public.oc_users%rowtype; v_app public.oc_provider_applications%rowtype;
...
begin
  ...
  if v_provider.provider_application_id is not null then select * into v_app from public.oc_provider_applications where id=v_provider.provider_application_id; end if;
  ...
  if v_provider.provider_application_id is not null then
    select count(*) filter(where required),count(*) filter(where required and status in ('passed','waived')) into v_required_checks,v_passed_checks from public.oc_provider_verification_checks where application_id=v_provider.provider_application_id;
  end if;
  ...
  if v_provider.is_available and private.oc_provider_core_dispatch_ready(v_provider.id) then
    select count(*) into v_offer_count from public.oc_booking_offers o join public.oc_bookings b on b.id=o.booking_id where o.provider_id=v_provider.id and o.status='pending' and o.expires_at>now() and b.provider_id is null and b.status in ('pending','matching') and private.oc_provider_service_dispatch_ready(v_provider.id,b.service_id);
  end if;
  select coalesce(sum(case when p.status='transferred' and coalesce(p.transferred_at,p.created_at)>=date_trunc('day',now()) then p.provider_amount else 0 end),0)/100.0,
  ...
  into v_today,v_week,v_total,v_pending from public.oc_booking_payments p where p.provider_id=v_provider.id;
