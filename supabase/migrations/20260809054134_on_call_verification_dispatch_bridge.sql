...
returns boolean language sql stable security definer set search_path='pg_catalog','public','private' as $$
  select coalesce((
    select u.role='provider' and u.status='active'
      and p.approval_status='active'
      and p.stripe_onboarding_complete and p.stripe_payouts_enabled
      and a.status='approved'
      and not exists(
        select 1 from public.oc_provider_verification_checks v
        where v.application_id=a.id
          and v.required
          and v.check_type not in ('license','insurance')
          and v.status not in ('passed','waived')
      )
    from public.oc_provider_profiles p
    join public.oc_users u on u.id=p.user_id
    join public.oc_provider_applications a on a.id=p.provider_application_id
...
returns boolean language sql stable security definer set search_path='pg_catalog','public','private' as $$
  select private.oc_provider_core_dispatch_ready(p_provider_id)
    and coalesce((
      ...
      join public.oc_service_catalog s on s.id=ps.service_id
...
language sql security definer set search_path='pg_catalog','public','private' as $$
  select a.id,a.application_number,p.id,a.first_name,a.last_name,a.email,v.check_type,v.required,v.status,v.evidence_urls,v.notes,v.reviewed_at,
    case v.check_type
      when 'identity' then p.identity_verified
      when 'background' then p.background_check_status='passed'
      when 'license' then p.license_verified
      when 'insurance' then p.insurance_verified
      when 'service_area' then p.service_area_verified
      else null
    end
  from public.oc_provider_applications a
  join public.oc_provider_verification_checks v on v.application_id=a.id
  left join public.oc_provider_profiles p on p.provider_application_id=a.id
  where private.is_marketplace_operator(auth.uid())
  order by case a.status when 'approved' then 0 when 'reviewing' then 1 else 2 end,a.created_at,v.check_type
$$;
revoke all on function public.oc_ops_provider_verifications() from public,anon;
grant execute on function public.oc_ops_provider_verifications() to authenticated;
...
begin
 ...
 update public.oc_provider_verification_checks
 set status=p_status,notes=nullif(left(trim(coalesce(p_notes,'')),2000),''),reviewed_by=auth.uid(),reviewed_at=case when p_status in ('passed','failed','waived','expired') then now() else reviewed_at end,updated_at=now()
 where application_id=p_application_id and check_type=p_check_type
 ...
 if v_provider_id is not null then
   update public.oc_provider_profiles set
     identity_verified=case when p_check_type='identity' then p_status in ('passed','waived') else identity_verified end,
     background_check_status=case when p_check_type='background' then case when p_status in ('passed','waived') then 'passed' when p_status='failed' then 'failed' else 'pending' end else background_check_status end,
     ...
     service_area_verified=case when p_check_type='service_area' then p_status in ('passed','waived') else service_area_verified end,
     is_available=case when p_status in ('failed','expired') and v_check.required then false else is_available end,
     updated_at=now()
...
revoke all on function public.oc_ops_review_provider_verification(uuid,text,text,text) from public,anon;
grant execute on function public.oc_ops_review_provider_verification(uuid,text,text,text) to authenticated;
...
begin
 ...
 select v_provider_id,m.service_id,true from public.oc_application_service_matches m join public.oc_service_catalog c on c.id=m.service_id and c.is_active where m.application_id=v_app.id
...
revoke all on function public.oc_provider_activate_approved_application() from public,anon;
grant execute on function public.oc_provider_activate_approved_application() to authenticated;
...
begin
 ...
 if not found then raise exception 'ON CALL provider account required' using errcode='42501'; end if;
 ...
 if p_lat is not null and (p_lat < -90 or p_lat > 90) then raise exception 'Invalid latitude'; end if; if p_lng is not null and (p_lng < -180 or p_lng > 180) then raise exception 'Invalid longitude'; end if; if p_accuracy_meters is not null and p_accuracy_meters<0 then raise exception 'Invalid location accuracy'; end if;
...
begin
 ...
 select p.* into v_provider from public.oc_provider_profiles p join public.oc_users u on u.id=p.user_id where u.auth_id=auth.uid() and u.role='provider' and u.status='active' and p.is_available limit 1;
 if not found or not private.oc_provider_core_dispatch_ready(v_provider.id) then return; end if;
...
begin
 select p.id into v_provider_id from public.oc_provider_profiles p join public.oc_users u on u.id=p.user_id where u.auth_id=auth.uid() and u.role='provider' and u.status='active' and p.is_available limit 1;
 if v_provider_id is null or not private.oc_provider_core_dispatch_ready(v_provider_id) then return; end if;
