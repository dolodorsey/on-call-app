...
begin
  ...
  set status=case when status in ('completed','canceled','cancelled') then status else 'canceled' end,
      ...
      service_address=null,service_city=null,service_state=null,service_zip=null,service_lat=null,service_lng=null,address=null,lat=null,lng=null,
      service_notes=null,access_instructions=null,before_photos=null,after_photos=null,notes=null,customer_notes=null,updated_at=now()
  ...
  update public.oc_disputes d set description=null,evidence_urls=null,internal_notes=null,resolution=case when status='open' then coalesce(resolution,'Account holder deleted profile; preserve financial review only.') else resolution end,updated_at=now()
  ...
  where customer_id=v_user_id or (v_provider_id is not null and provider_id=v_provider_id) or exists(select 1 from public.oc_bookings b where b.id=i.booking_id and (b.customer_id=v_user_id or (v_provider_id is not null and b.provider_id=v_provider_id)));
  update public.oc_provider_reviews set review_text=null,provider_response=null,is_featured=false
  where customer_id=v_user_id or (v_provider_id is not null and provider_id=v_provider_id);

  update public.oc_support_tickets
  set subject='Deleted account support record',description='',operator_note=null,status='closed',resolved_at=coalesce(resolved_at,now()),updated_at=now()
  ...
  if v_provider_id is not null then
    ...
    update public.oc_provider_profiles set skills='{}'::text[],is_available=false,background_check_status='withdrawn',approval_status='deactivated',identity_verified=false,license_verified=false,insurance_verified=false,service_area_verified=false,vehicle_type=null,vehicle_info='{}'::jsonb,stripe_charges_enabled=false,stripe_payouts_enabled=false,stripe_onboarding_complete=false,deactivated_at=coalesce(deactivated_at,now()),updated_at=now() where id=v_provider_id;
  end if;

  update public.oc_users
  set auth_id=null,first_name='Deleted',last_name='Account',full_name='Deleted account',email=null,phone=null,phone_verified=false,avatar_url=null,address=null,city=null,state=null,state_code=null,zip_code=null,lat=null,lng=null,date_of_birth=null,status='deleted',stripe_customer_id=null,referral_code='deleted_'||replace(id::text,'-',''),referred_by=null,preferred_payment=null,notification_prefs='{"sms":false,"push":false,"email":false}'::jsonb,last_active_at=null,updated_at=now()
  where id=v_user_id;

  return jsonb_build_object('ok',true,'found',true,'oc_user_id',v_user_id,'provider_id',v_provider_id,'bookings_anonymized',v_booking_count,'support_cases_scrubbed',v_support_count,'profile_anonymized',true);
end;
$$;